...
function SectionHeader({ title, action = "Ver todos" }: { title: string; action?: string }) {
  return (
    <div className="mb-3 flex items-center justify-between px-5 lg:px-0">
      <h2 className="text-[22px] font-black tracking-tight text-ink lg:text-2xl">{title}</h2>
...
function GameCard({ game, compact = false, onPlay }: { game: Game; compact?: boolean; onPlay: (game: Game) => void }) {
  ...
  return (
    <article
      ...
      } ${compact ? "h-[246px] w-[154px] min-[390px]:w-[166px] lg:h-[258px] lg:w-full" : "h-[286px] w-[182px] min-[390px]:w-[190px] lg:h-[306px] lg:w-full"}`}
    >
      <button className="flex h-full w-full flex-col text-left" onClick={() => onPlay(game)}>
        <div className={`${compact ? "h-[112px] lg:h-[126px]" : "h-[124px] lg:h-[142px]"} relative overflow-hidden bg-zinc-100`}>
          <Image src={getGameImage(game)} alt="" fill sizes="(max-width: 1024px) 190px, 220px" className="object-cover" priority={game.daily_challenge} />
...
function HorizontalGames({
  ...
  return (
    <section className="py-3 lg:py-5">
      ...
      <div className="hide-scrollbar flex gap-3 overflow-x-auto px-5 pb-2 lg:grid lg:grid-cols-4 lg:gap-4 lg:overflow-visible lg:px-0 xl:grid-cols-5">
        {games.map((game) => (
...
function DailyChallenge({ daily, onPlay }: { daily: Game; onPlay: (game: Game) => void }) {
  return (
    <section className="mx-4 mt-5 overflow-hidden rounded-lg border border-teal-500/35 bg-white shadow-ranked min-[390px]:mx-5 lg:mx-0 lg:mt-6">
      <div className="grid min-h-[226px] grid-cols-[1.04fr_.96fr] max-[374px]:grid-cols-1 lg:min-h-[330px] lg:grid-cols-[1.05fr_.95fr]">
        <div className="flex flex-col justify-between p-4 min-[390px]:p-5 lg:p-8">
          <div>
            ...
            <h1 className="text-[23px] font-black leading-tight tracking-tight text-ink min-[390px]:text-[25px] lg:max-w-md lg:text-5xl">{daily.title}</h1>
            <p className="mt-2 text-[13px] leading-6 text-zinc-600 min-[390px]:text-sm lg:mt-4 lg:max-w-lg lg:text-base lg:leading-7">{daily.short_description}</p>
          </div>
          <div className="mt-4 space-y-3">
            ...
            <button
              ...
              className="flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-action text-sm font-black text-white shadow-soft transition active:scale-[.98] lg:w-56"
            >
        ...
        <div className="relative min-h-full overflow-hidden bg-teal-700 max-[374px]:h-40 max-[374px]:min-h-0">
          <Image
            ...
            alt=""
            fill
            sizes="(max-width: 430px) 48vw, 520px"
...
function CategoryTabs({
  ...
  return (
    <div className="mx-4 mt-5 grid h-12 grid-cols-4 overflow-hidden rounded-lg border border-line bg-white text-sm font-black shadow-soft min-[390px]:mx-5 lg:mx-0 lg:max-w-xl">
...
function HomeView({
  ...
  return (
    <>
      <header className="px-5 pt-7 lg:px-0 lg:pt-8">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[23px] font-black leading-tight tracking-tight text-ink min-[390px]:text-[26px] lg:text-4xl">
              Só Mais Uma Fase
            </p>
            <p className="mt-1 text-[13px] font-medium text-muted min-[390px]:text-sm lg:text-base">
      ...
      <section className="px-5 py-4 lg:hidden">
        <button
      ...
      <section className="hidden py-5 lg:grid lg:grid-cols-[1fr_360px] lg:gap-5">
        <button
          onClick={() => setTab("ranking")}
          className="flex items-center justify-between rounded-lg border border-teal-500/25 bg-zinc-950 p-5 text-left text-white shadow-ranked"
        >
          <div className="flex items-center gap-4">
            <div className="flex h-14 w-14 items-center justify-center rounded-lg bg-teal-400/15 text-teal-200">
              <UsersThree size={30} weight="bold" />
            </div>
            <div>
              <p className="text-lg font-black">Comunidade em disputa</p>
              <p className="text-sm font-bold text-zinc-300">Top 3, rankings diários e Hall da Fama dos desafios oficiais.</p>
            </div>
          </div>
      ...
      <section className="py-4 lg:py-5">
        ...
        <div className="hide-scrollbar flex gap-3 overflow-x-auto px-5 pb-2 lg:grid lg:grid-cols-3 lg:overflow-visible lg:px-0">
          {hallOfFame.map((item) => (
            <button
              key={item.title}
              onClick={() => setTab("hall")}
              className="w-[230px] shrink-0 rounded-lg border border-amber-300/60 bg-white p-4 text-left shadow-soft lg:w-full"
            >
...
function RankingView() {
  ...
  return (
    <main className="px-5 pb-32 pt-7 lg:mx-auto lg:max-w-5xl lg:px-0 lg:pb-12 lg:pt-8">
      <div className="mb-5">
...
function QuickView({ games, onPlay }: { games: Game[]; onPlay: (game: Game) => void }) {
  ...
  return (
    <main className="px-5 pb-32 pt-7 lg:mx-auto lg:max-w-6xl lg:px-0 lg:pb-12 lg:pt-8">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4 lg:gap-4 xl:grid-cols-5">
...
function ProfileView() {
  return (
    <main className="px-5 pb-32 pt-7 lg:mx-auto lg:max-w-2xl lg:px-0 lg:pb-12 lg:pt-8">
...
function DesktopNav({ tab, setTab }: { tab: Tab; setTab: (tab: Tab) => void }) {
  return (
    <nav className="sticky top-0 z-40 hidden border-b border-line bg-paper/95 py-4 backdrop-blur lg:block">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-lg font-black text-ink">Só Mais Uma Fase</p>
          <p className="text-xs font-bold text-muted">Portal de jogos rápidos</p>
        </div>
        <div className="flex items-center gap-2 rounded-lg border border-line bg-white p-1 shadow-soft">
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex h-10 items-center gap-2 rounded-md px-4 text-sm font-black ${
                  active ? "bg-teal-500/10 text-teal-700" : "text-zinc-600 hover:bg-zinc-50"
                }`}
              >
...
export function GamePortal({ games }: { games: Game[] }) {
  ...
  if (selectedGame) {
    return (
      <div className="mx-auto min-h-screen w-full max-w-[430px] bg-paper shadow-[0_0_0_1px_rgba(228,228,231,.7)] max-[500px]:mx-0 lg:max-w-5xl lg:shadow-none">
  ...
  return (
    <div className="mx-auto min-h-screen w-full max-w-[430px] bg-paper pb-36 shadow-[0_0_0_1px_rgba(228,228,231,.7)] max-[500px]:mx-0 lg:max-w-6xl lg:px-6 lg:pb-12 lg:shadow-none">
      ...
      <nav className="fixed bottom-0 left-1/2 z-50 grid h-[76px] w-full max-w-[430px] -translate-x-1/2 grid-cols-5 border-t border-line bg-white/96 px-2 pb-2 pt-1 shadow-[0_-12px_30px_rgba(24,24,27,.08)] backdrop-blur max-[500px]:left-0 max-[500px]:translate-x-0 lg:hidden">
        {navItems.map((item) => {
          const Icon = item.icon;
          const active = tab === item.id;
          return (
